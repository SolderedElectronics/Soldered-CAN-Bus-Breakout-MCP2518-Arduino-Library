...
FLASH<uint8_t> JTAG2::sgn_resp[29]{0x86,
                                   1,
                                   1,
                                   PARAM_FW_VER_M_MIN_VAL,
                                   PARAM_FW_VER_M_MAJ_VAL,
                                   PARAM_HW_VER_M_VAL,
                                   1,
                                   PARAM_FW_VER_S_MIN_VAL,
                                   PARAM_FW_VER_S_MAJ_VAL,
                                   PARAM_HW_VER_S_VAL,
                                   0,
                                   0,
                                   0,
...
{
    // Reset the MCU now, to prevent the WDT (if active) to reset it at an
...
void NVM_buffered_write(const uint16_t address, const uint16_t length, const uint8_t buff_size,
                        const uint8_t write_cmnd)
{
    ...
    // Sends a block of bytes from the command body to memory, using the UPDI
    // interface On entry, the UPDI pointer must already point to the desired
    // address On exit, the UPDI pointer points to the next byte after the last
    // one written Returns updated index into the command body, pointing to the
    // first unsent byte.
