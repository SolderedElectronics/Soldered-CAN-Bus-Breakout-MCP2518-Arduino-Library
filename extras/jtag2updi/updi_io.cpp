...
// Enable to get pulses on PD7 showing the sample times for the software UART
// input
//#define _DEBUG
...
uint8_t UPDI_io::put(char c)
{
    ...
    parity = 0;
    /* If we can be sure an overflow has happened by now due to instruction
     * latency, */
...
uint8_t UPDI_io::get()
{
    ...
    /* Must disable pull-up, because the UPDI UART just sends very short output
     * pulses at the beginning of each bit time. */
    /* If pull up is enabled, there will be a drift to high state that results in
     * erroneous input sampling. */
    /* As a side effect, random electrical fluctuations of the input prevent an
     * infinite wait loop */
