...
uint8_t UPDI_io::get()
{
    ...
    __asm volatile(" ldi  %0, 0 \n\t" // init
                   ...
                   : "=r"(c)
                   : [uart_port] "i"(_SFR_IO_ADDR(PIN(UPDI_PORT))), [uart_pin] "i"(UPDI_PIN), [rxdelay] "i"(RXDELAY),
                     [rxhalfd] "i"(RXHALFD)
...
uint8_t UPDI_io::put(char c)
{
    ...
    __asm volatile(
        ...
        :
        : [uart_port] "i"(_SFR_IO_ADDR(PORT(UPDI_PORT))), [uart_pin] "i"(UPDI_PIN), [txdelay] "i"(TXDELAY), [ch] "r"(c)
